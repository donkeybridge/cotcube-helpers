  ...
  module Helpers
    ...
    def symbols(config: init, type: nil, symbol: nil, id: nil)
      if config[:symbols_file].nil?
        ...
        CSV
          .read(config[:symbols_file], headers: %i{ id symbol ib_symbol internal exchange currency ticksize power months type bcf reports format name})
          .map{|row| row.to_h }
          .map{|row|
    ...
    def micros(config: init, symbol: nil, id: nil)
      if config[:micros_file].nil?
        ...
        CSV
          .read(config[:micros_file], headers: %i{ id symbol ib_symbol internal exchange currency ticksize power months type bcf reports format name})
          .map{|row| row.to_h }
          .map{|row|
            [ :ticksize, :power, :bcf ].each {|z| row[z] = row[z].to_f }
            row[:format] = "%#{row[:format]}f"
